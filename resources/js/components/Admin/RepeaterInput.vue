            ...
            >
                ...
                <div class="p-3">
                    <file-input
                        ...
                        :selectEventName="entityName"
                        :uploadEventName="entityName"
                        :filesConfig="{parentEntityIndex: entityIndex, selectMode: 'single', fileTypes: 'image'}"
                    ...
                    <div>
                        <text-input v-model="entity.subtitle" id="entity_subtitle" label="Год" />
                    </div>
                    <div>
                        <text-input v-model="entity.description" id="entity_description" label="Описание" />
                    </div>
                    <div>
                        <tiptap v-model="entity.text_body"  id="text_body" label="Текст произведения" />
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            eventBus.$on('selectedFiles' + this.entityName, (files, entity_index) => {
        ...
        methods: {
            onSelectedFilesEntity(files, entity_index) {
                this.entities[entity_index].file = files[0]
                this.entities[entity_index].file_id = files[0].id
                this.$emit('onUpdateEntities', this.entities)
            },
            ...
            openEntityFilesModal(entity_name, files_config) {
                eventBus.$emit('openFilesModal', this.entityName, files_config)
                this.$emit('onUpdateEntities', this.entities)
            },
