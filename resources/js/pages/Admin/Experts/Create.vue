                    ...
                    <form @submit.prevent="store">
                        <div class="p-6">
                            ...
                            <div class="mb-6">
                                ...
                                <div v-if="errors.expert_files" class="mt-2 text-red-400">{{ displayExpertFilesError }}</div>
                            </div>
    ...
    export default {
        ...
        mounted() {
            ...
            eventBus.$on('selectedFiles', (files) => {
                this.files = files
                this.form.expert_files = []
                for(var file of files) {
                    this.form.expert_files.push(file.id)
                }
            })
        },
        computed: {
            displayExpertCategoriesError() {
                return (Array.isArray(this.errors.category_id)) ? this.errors.category_id.join('; ') : this.errors.category_id
            },
            displayExpertFilesError() {
                return (Array.isArray(this.errors.expert_files)) ? this.errors.expert_files.join('; ') : this.errors.expert_files
            },
        },
        methods: {
            setData() {
                axios.get('/api/admin/experts/create')
                .then(res => {
            ...
            store() {
                ...
                axios.post('/api/admin/experts', this.form)
                .then(res => {
                    this.errors = {}
                    eventBus.$emit('flash-message', 'success', res.data.success)
                    this.$router.push({name: 'admin-experts-index'})
                })
                .catch(err => {
                    this.errors = {}
            ...
            deleteFile(index) {
                ...
                this.form.expert_files.splice(index, 1)
            },
            deleteFiles() {
                this.files = []
                this.form.expert_files = []
