<template>
    <div>
        <h1 class="px-8 font-semibold text-xl text-gray-800 leading-tight">
            Видеостудия / Перевод / Редактирование
        </h1>
        <div class="px-8 py-8">
...
<script>
    ...
    export default {
        ...
        methods: {
            setData() {
                axios.get('/api/admin/video_articles/video_articles_translations/' + this.$route.params.id + '/edit')
                .then(res => {
                    this.video_article = res.data.video_article
                    this.form.title = res.data.video_article_translation.title
                    this.form.description = res.data.video_article_translation.description
                })
                .finally(() => this.isloading = false)
            },
            store() {
                this.isStoreloading = true
                axios.put('/api/admin/video_articles/video_articles_translations/' + this.$route.params.id, this.form)
                .then(res => {
                    this.errors = {}
