                ...
                <div v-else>
                    <form @submit.prevent="store">
                        <div class="p-6">
                            ...
                            <button-primary type="submit" :isLoading="isStoreloading">Изменить</button-primary>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            setData() {
                axios.get('/api/admin/tags/' + this.$route.params.id + '/edit')
                .then(res => {
                    this.form.title = res.data.tag.title
                })
                .finally(() => this.isloading = false)
            },
            store() {
                this.isStoreloading = true
                axios.put('/api/admin/tags/' + this.$route.params.id, this.form)
                .then(res => {
                    this.errors = {}
