    ...
    <div>
        ...
        <div class="px-8 py-8">
            ...
            <div v-for="author in authors.data" :key="author.id">
                <h2 class="text-xl mb-4">{{ author.title }}</h2>
                <h2>Общее кол-во саписей: {{ author.total_count }}</h2>
                <div v-if="author.posts.length > 0" class="bg-white rounded-md shadow overflow-x-auto">
                    <table class="table-fixed w-full">
                        <tr class="text-left font-bold">
                            <th class="pl-6 pr-3 py-3 w-52">Дата создания</th>
                            <th class="px-3 py-3 max-w-xs">Заголовок</th>
                        </tr>
                        <tr v-for="post in author.posts" :key="post.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                            <td class="border-t pl-6 pr-3 py-3">
                                {{ post.published_at | displayDateTime }}
                            </td>
                            <td class="border-t px-3 py-3">
                                <router-link :to="'/admin/posts/' + post.id + '/edit'" target="_blank" class="text-sm text-blue-400">{{ post.title }}</router-link>
                            </td>
                        </tr>
                    </table>
                </div>
                <div v-else>
                    <div class="bg-white border-t px-4 py-3">Нет новостей за период.</div>
                </div>
            </div>
            <div v-if="empty_data">
    ...
    export default {
        ...
        methods: {
            ...
            setData(page = '') {
                ...
                axios.get('/api/admin/statistics', {params: queryParams})
                .then(res => {
